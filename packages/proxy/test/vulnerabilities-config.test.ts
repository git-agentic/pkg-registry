...
// readable-but-CORRUPT SENTINEL_VULNERABILITIES file (parity with SENTINEL_ADVISORIES),
// not just an unreadable one, and that a legitimately empty vuln list still boots.
describe("proxy boot with SENTINEL_VULNERABILITIES (child process)", () => {
  const dir = mkdtempSync(join(tmpdir(), "sentinel-vulnerabilities-config-"));
  ensureFixtures();

  function bootWith(vulnerabilitiesPath: string | undefined): Promise<{ code: number; stderr: string }> {
    const env = { ...process.env, SENTINEL_UPSTREAM: "fixtures", SENTINEL_BOOT_EXIT: "1", SENTINEL_PORT: "0" };
    if (vulnerabilitiesPath) env.SENTINEL_VULNERABILITIES = vulnerabilitiesPath; else delete env.SENTINEL_VULNERABILITIES;
  ...
  test("non-JSON vulnerabilities file → non-zero exit + FATAL message", async () => {
  ...
  test("unset SENTINEL_VULNERABILITIES → boots fine, exit 0", async () => {
