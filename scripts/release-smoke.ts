...
//   - internal @sentinel/* dependencies resolve from the packed tarballs only
//
...
for (const ws of WORKSPACES) {
  ...
  results.tarballs.push({ name: `@sentinel/${ws}`, file: info.filename, bytes: info.size, sha256, files: info.entryCount, unpacked: info.unpackedSize });
...
const fileDep = (ws: string) => `file:${join(packDir, results.tarballs.find((t) => t.name === `@sentinel/${ws}`)!.file)}`;
const pkgJson = {
  name: "sentinel-release-smoke", private: true, version: "0.0.0", type: "module",
  dependencies: Object.fromEntries(WORKSPACES.map((ws) => [`@sentinel/${ws}`, fileDep(ws)])),
  ...
  overrides: { "@sentinel/core": fileDep("core"), "@sentinel/proxy": fileDep("proxy"), "@sentinel/sandbox": fileDep("sandbox") },
};
...
check("internal deps resolved from tarballs (not registry)", () => {
  ...
  const bad = Object.entries(lock.packages).filter(([k, v]) => k.includes("@sentinel/") && v.resolved && !v.resolved.startsWith("file:"));
  if (bad.length) throw new Error(`registry-resolved: ${bad.map(([k]) => k).join(", ")}`);
  return "all @sentinel/* resolved file:";
});
...
console.log(`\n[3/5] imports + types`);
for (const ws of WORKSPACES) {
  check(`import @sentinel/${ws}`, () => {
    run(process.execPath, ["-e", `import("@sentinel/${ws}").then((m)=>{ if(!m || typeof m !== "object") throw new Error("empty module") })`], { cwd: proj });
    return "";
  });
}
check("ENGINE_VERSION matches release", () => {
  const v = run(process.execPath, ["-e", `import("@sentinel/core").then((m)=>console.log(m.ENGINE_VERSION))`], { cwd: proj }).trim();
...
check("type declarations resolve (tsc --noEmit, NodeNext)", () => {
  ...
  writeFileSync(join(proj, "typecheck.ts"), [
    `import { runAudit, score, DEFAULT_POLICY, type AuditReport, type EnterprisePolicy } from "@sentinel/core";`,
    `import { createServer, NpmUpstream, type Upstream } from "@sentinel/proxy";`,
