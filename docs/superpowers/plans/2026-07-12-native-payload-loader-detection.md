...
**Tech Stack:** Node 24 + TypeScript, npm workspaces (`core`, `proxy`, `sandbox`, `cli`), Express 5, `tar` 7, `acorn` + `acorn-walk` (new — `@agentic-sentinel/core`'s first parser deps), tests on `node:test` + `tsx`.
...
import { matchPackage, type AuditReport, type EnterprisePolicy } from "@agentic-sentinel/core";
...
import { parsePolicy } from "@agentic-sentinel/core";
