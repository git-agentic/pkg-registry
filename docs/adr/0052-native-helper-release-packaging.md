...
For `0.1.0-alpha.1`, `@sentinel/sandbox` ships the helper **as source only**,
...
3. **Compilation is an explicit operator action, never a lifecycle script.**
   `node node_modules/@sentinel/sandbox/scripts/build-native.mjs` compiles the
...
- **Architecture-specific optional packages (`@sentinel/landlock-linux-x64`,
...
- **Blocking publication of `@sentinel/sandbox` (and its dependents).**
