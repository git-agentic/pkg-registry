...
**Enforcement scope — `process` exec floor enforced on macOS, and on Linux where
Landlock + the from-source helper are available (advisory otherwise); the
exfil-tool carve-out enforced on both; `native` advisory by decision.** The
...
(no `noexec` mount option), so Phase 29 shipped only the exfil-tool carve-out —
each `SENSITIVE_EXECUTABLES` literal masked with `--ro-bind /dev/null <literal>`
unless a `process:` Grant lifts it (ADR-0043). Phase 2 (Landlock) closes the
floor gap where the kernel and toolchain allow it: a first-party, from-source
`landlock-exec` helper (`packages/sandbox/native/landlock-exec.c`, compiled by a
`npm run build` step — `build-native.mjs`, no-op off Linux/no-`cc`) is invoked
inside bwrap as `landlock-exec --allow <floor> -- /bin/sh -c <script>`,
applying `LANDLOCK_ACCESS_FS_EXECUTE` over `linuxExecFloor` — `execAllowFloor`
plus `/lib`, `/lib64`, `/usr/lib`, `/usr/lib64` (library/linker `mmap` access) —
before exec'ing the lifecycle script; a dropped binary outside that floor is
kernel-denied. Detection is fail-open and pre-checked: the helper is used iff it
exists AND `landlock-exec --check` (an ABI probe) exits 0, cached per process;
...
distinguishes loading an artifact from reading it. The projectRoot-in-floor
